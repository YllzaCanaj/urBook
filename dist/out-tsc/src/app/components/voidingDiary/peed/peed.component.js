...
let PeedComponent = class PeedComponent {
    ...
    ngOnInit() {
        ...
        this.sqlite.create({
            ...
            db.executeSql('SELECT * FROM  pees', []).then((data) => {
                console.log("pee here", data);
                if (data.rows.length > 0) {
                    for (let i = 0; i < data.rows.length; i++) {
                        console.log('item: ', data.rows.item(i));
                        this.pee.push(data.rows.item(i));
                        this.pee[i].CREATED_AT = this.time_ago(new Date(this.pee[i].CREATED_AT));
                    }
...
PeedComponent = tslib_1.__decorate([
    Component({
        selector: 'app-peed',
        templateUrl: './peed.component.html',
        styleUrls: ['./peed.component.scss'],
    }),
    tslib_1.__metadata("design:paramtypes", [TranslateService, Storage, SQLite])
], PeedComponent);
export { PeedComponent };
//# sourceMappingURL=peed.component.js.map